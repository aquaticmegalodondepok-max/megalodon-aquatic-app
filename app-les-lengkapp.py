...
# ----------------------------------------------------------------------
# 7. KELOLA, EDIT & HAPUS DATA PELATIH (ADMIN KHUSUS)
# ----------------------------------------------------------------------
elif menu == "👥 Kelola & Hapus Data Pelatih":
    st.title("👥 Kelola, Edit, Tambah & Hapus Data Pelatih (Khusus Admin)")
    
    tab_p1, tab_p2, tab_p3 = st.tabs(["✏️ Edit Master & Tarif Pelatih", "➕ Tambah Pelatih Baru", "🗑️ Hapus Pelatih Tertentu"])
    
    with tab_p1:
        st.subheader("📋 Tabel Master Pelatih (Edit Tarif / Nominal Gaji Sesi)")
        st.info("💡 **Petunjuk:** Ubah angka 'Tarif/Sesi' langsung pada tabel untuk mengubah nominal gaji pelatih.")
    ...
    with tab_p2:
        st.subheader("➕ Tambah Pelatih / Coach Baru")
        with st.form("form_tambah_pelatih"):
            col_tp1, col_tp2 = st.columns(2)
            with col_tp1:
                new_plt_id = f"PLT-{len(st.session_state.pelatih) + 1:03d}"
                st.text_input("ID Pelatih", value=new_plt_id, disabled=True)
                new_plt_nama = st.text_input("Nama Lengkap Pelatih")
            with col_tp2:
                new_plt_hp = st.text_input("No HP (WhatsApp)", "08123456789")
                new_plt_tarif = st.number_input("Tarif Gaji per Sesi (Rp)", value=75000, step=5000)
                
            if st.form_submit_button("Simpan Pelatih Baru"):
                if new_plt_nama.strip() != "":
                    row_p_baru = pd.DataFrame([{
                        "ID": new_plt_id,
                        "Nama": new_plt_nama.strip(),
                        "No HP": new_plt_hp,
                        "Tarif/Sesi": new_plt_tarif,
                        "Status": "Aktif"
                    }])
                    st.session_state.pelatih = pd.concat([st.session_state.pelatih, row_p_baru], ignore_index=True)
                    st.success(f"Pelatih baru **{new_plt_nama}** berhasil ditambahkan!")
                    st.rerun()
                else:
                    st.error("Nama Pelatih tidak boleh kosong!")
